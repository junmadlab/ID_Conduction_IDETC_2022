...
r_top = preds[71*gg:71*(gg+1),71:142] #RF prediction for given volf and wid
#----------------------------------------------------------------------------------------------------
...
filename = "r_v="+str(vol_f)+"_w="+str(wis)+"_s="+str(uu)#filetype added later
...
t = tm.time()
print("Starting optimization for RF model, v = "+str(vfs)+", w = "+str(wis)+", run "+ str(uu)+".")
a_opt = solver.solve()
t = tm.time() - t
...
filename = "r_v="+str(vol_f)+"_w="+str(wis)+"_s="+str(uu)+".txt" #filetype added later
text = open(filename)
text = text.read()
...
if uu==0:
    ...
    results = np.load("r_v="+str(vol_f)+"_w="+str(wis)+"_"+str(gg)+".npy")
r_cn = np.load('r_cn.npy')
cn = r_cn[0]
...
filename3 = "r_v="+str(vol_f)+"_w="+str(wis)+ "_s="+str(uu)+".txt"
os.remove(filename3)
                
filename2 = "r_v="+str(vol_f)+"_w="+str(wis)+"_"+str(gg)+".npy"
...
if uu==4: #after the end of a set of runs for a given image
    ...
    print("RF Trajectories Progress: " + str(progress) + "%.")
